...
shinyUI(fluidPage
        ...
        sidebarPanel(
          ...
          tabsetPanel(type="tabs",
            tabPanel("Summary plots", column(12, align="center", br(), verbatimTextOutput(outputId= 'table1.output'), br(), br(), plotOutput('plot_sum2.output', width = "40%"),downloadButton(outputId = "sum1download.output", label = "Download the distribution of counts per sample"), br(), br(), plotOutput('plot_sum3.output', width = "50%"),downloadButton(outputId = "sum2download.output", label = "Download the detected transcripts per sample"), br(), br(),br() )),
            tabPanel("Clustering plots", column(12, align="center", br(), br(), plotOutput('plot1.output', width = "50%"),downloadButton(outputId = "pca1download.output", label = "Download the MDS plot"), br(), br(), plotOutput('plot2.output', width = "80%"),downloadButton(outputId = "pca2download.output", label = "Download the Principal components plot for all samples"), br(), br(), plotOutput('plot3.output', width = "80%"),downloadButton(outputId = "pca3download.output", label = "Download the CPM variance between housekeeping genes"),  br(), br(),br() )),
            tabPanel("Correlational plots",column(12, align="center", br(), br(), plotOutput('plot4.output', width = "80%", height = "400px"),downloadButton(outputId = "hierdownload.output", label = "Download the hierarchichal plot"), br(), br(), plotOutput('plot5.output', width = "80%", height = "500px"),downloadButton(outputId = "cordownload.output", label = "Download the pearson correlation plot"), br(), br(), plotOutput('plot6.output', width = "50%"),downloadButton(outputId = "wtdownload.output", label = "Download the weights per sample plot"),  br(), br(),br() ))
            ))
)
